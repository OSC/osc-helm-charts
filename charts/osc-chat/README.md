...
| database.postgresql.primary.initdb.scripts."extra-grants.sql" | string | `"GRANT EXECUTE ON FUNCTION pg_ls_waldir() TO {{ .Values.auth.username }};\n"` |  |
...
| qdrant.snapshot.cronjob.enabled | bool | `true` |  |
| qdrant.snapshot.cronjob.resources.limits.cpu | int | `1` |  |
| qdrant.snapshot.cronjob.resources.limits.memory | string | `"512Mi"` |  |
| qdrant.snapshot.cronjob.resources.requests.cpu | string | `"250m"` |  |
| qdrant.snapshot.cronjob.resources.requests.memory | string | `"256Mi"` |  |
| qdrant.snapshot.cronjob.schedule | string | `"0 0 * * *"` |  |
